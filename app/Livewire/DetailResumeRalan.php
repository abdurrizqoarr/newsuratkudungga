...
use App\Models\ResumePasienRalan;
...
class DetailResumeRalan extends Component
{
    public $norawat;
    public $resume;
    public $nik;
    ...
    {
        $this->signedFile = SignDokumen::where('no_rawat', 'ralan_' . $this->norawat)->first();
    }

    public function loadResume()
    {
        return ResumePasienRalan::select(
            'resume_pasien.*',
            ...
            ->where('resume_pasien.no_rawat', request()->no_rawat)
            ->join('reg_periksa', 'resume_pasien.no_rawat', '=', 'reg_periksa.no_rawat')
            ->join('pasien', 'reg_periksa.no_rkm_medis', '=', 'pasien.no_rkm_medis')
            ->join('poliklinik', 'reg_periksa.kd_poli', '=', 'poliklinik.kd_poli')
            ->join('dokter as dokter_dpjb', 'resume_pasien.kd_dokter', '=', 'dokter_dpjb.kd_dokter')
            ->first();
    }

    public function signResumeRalan()
    {
        $this->validate([
            'nik' => 'required|string',
        ...
        try {
            $resumeMedisRalan = ResumePasienRalan::select(
                'resume_pasien.*',
                'reg_periksa.tgl_registrasi',
                'reg_periksa.no_rkm_medis',
                'pasien.nm_pasien',
                ...
                ->where('resume_pasien.no_rawat', $this->norawat)
                ->join('reg_periksa', 'resume_pasien.no_rawat', '=', 'reg_periksa.no_rawat')
                ->join('pasien', 'reg_periksa.no_rkm_medis', '=', 'pasien.no_rkm_medis')
                ->join('poliklinik', 'reg_periksa.kd_poli', '=', 'poliklinik.kd_poli')
                ->join('dokter as dokter_dpjb', 'resume_pasien.kd_dokter', '=', 'dokter_dpjb.kd_dokter')
    ...
    public function downloadFile()
    {
        try {
            ...
            $filename = 'sign_resume_ralan_' . $this->nama_pasien . '_' . now()->timestamp . '.pdf';
    ...
    {
        return view('livewire.detail-resume-ralan');
    }
}
