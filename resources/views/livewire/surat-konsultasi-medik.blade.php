            ...
            <thead class="bg-gray-50 sticky top-0 shadow">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Lama
                        Uraian Konsultasi</th>
                </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-100">
                @forelse ($dataSurat as $surat)
                    <tr class="hover:bg-emerald-100 cursor-pointer"
                        @if ($surat->jawaban_diagnosa_kerja) onclick="window.open('{{ route('surat.konsultas-medik.print', $surat->no_permintaan) }}', '_blank')" @endif>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $surat->no_permintaan }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                            @if ($surat->jawaban_diagnosa_kerja)
                                <p class="font-bold text-green-600 text-lg">SUDAH</p>
                            @else
                                <p class="font-bold text-red-600 text-lg">BELUM</p>
                            @endif
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $surat->dokter_pengirim }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $surat->dokter_dikonsuli }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                            {{ $surat->tanggal ?? '-' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-700">
                            {{ $surat->diagnosa_kerja }}</td>
                        <td class="px-6 py-4 text-sm text-gray-700">{{ $surat->uraian_konsultasi }}
                        </td>
                    </tr>
                @empty
                    <tr>
